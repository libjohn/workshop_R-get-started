...
Often our goal is to reshape our data frame into a *tidy data* [@wickham2014] format, frequently known as **tall data**. We can use the {[tidyr](https://tidyr.tidyverse.org/)} package to reshape data into tall or wide formats. When coding in the tidyverse context, tall data is much easier to iterate over --- without ever writing an for loop or other kind of flow control. Later, in the [advance section, we'll introduce the {purrr} package](purrr.html "purrr") for powerful iteration techniques.

[![Tidy data from the book R for Data Science](images/tidy_data.png){fig-alt="Tidy data"}](images/tidy_data.png)

See Also the [*Pivot Vignette*](https://tidyr.tidyverse.org/articles/pivot.html "pivot_longer or pivot_wider"):
